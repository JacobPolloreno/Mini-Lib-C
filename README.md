...
| [ft_strsubdel.c](https://github.com/JacobPolloreno/42LIBFT/tree/master/srcs/ft_strsubdel.c) | ```char *ft_strsubdel(char **s, unsigned int start, size_t len);``` |  Return a substring beginning from start index. Behavior is undefined if start+len is not a substring. Free passed source string |
