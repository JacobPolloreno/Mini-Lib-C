...
/*   Updated: 2018/03/20 21:02:43 by jpollore         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			void *(*updatef)(void **, void *))
{
	...
	if ((*cmpf)(data_ref, (*root)->item) < 0)
		(*root)->left = btree_remove_node(&(*root)->left, data_ref, cmpf, delf, updatef);
	else if ((*cmpf)(data_ref, (*root)->item) > 0)
		(*root)->right = btree_remove_node(&(*root)->right, data_ref, cmpf, delf, updatef);
	else
	{
		if (!(*root)->left)
		{
